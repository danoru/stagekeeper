...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  if (req.method === "GET") {
    const plays = await prisma.plays.findMany({ orderBy: { title: "asc" } });
  ...
  if (req.method === "POST") {
    ...
    try {
      ...
      if (e.code === "P2002")
        return res.status(409).json({ error: "A play with that title already exists." });
      return res.status(500).json({ error: "Failed to create play." });
    }
  }

  if (req.method === "PUT") {
    const { id, title, writtenBy, premiere, duration, playbill } = req.body;
    if (!id) return res.status(400).json({ error: "ID is required." });
    try {
      const play = await prisma.plays.update({
        where: { id: Number(id) },
        data: {
          title,
          writtenBy: writtenBy || "",
